...
>>>>>>> 300ac5609cffcc71f6693d7c991720f60b5ee5a1
...
<<<<<<< HEAD
   ...
   {payload.role === "ADMIN" && (
    <p className="dashboard" onClick={() => navigate("/dashboard", { replace: true })} >
        Dashboard
    </p>
    )}
    {payload.role !== "CLIENT" && (
      <p className="courses" onClick={() => navigate("/courses", { replace: true })} >
       {payload.role === "STUDENT"? "Your Courses" : "Courses"}
      </p>
     )}
     {payload.role === "ADMIN" && (
       <p className="projects" onClick={() => navigate("/project", { replace: true })} >
         Projects
       </p>
      )}
     </div>
